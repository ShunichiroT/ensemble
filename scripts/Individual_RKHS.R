...
for (m in 2:length(pat)){
  if (m == 4 || m == 14){
    ...
    for (p in 1:1){
      ...
      G = tcrossprod(data.matrix(data_qtl))
      y=data_pheno
      ...
      for (k in (cnt_sample-500+1):cnt_sample){
        ...
        for (i in 1:REPEAT){
          fm=BGLR(y=y_test,ETA=list(G=list(K=G,model='RKHS')),
          ...
          setwd(SAVE_PATH)
          write.csv(record, paste("RKHS_0.8_imputed_concat.csv",sep=''), row.names=FALSE)
          write.csv(result_prediction, paste("RKHS_0.8_imputed_concat_result.csv",sep=''), row.names=FALSE)
          write.csv(result_prediction_test, paste("RKHS_0.8_imputed_test_concat_result.csv",sep=''), row.names=FALSE)
