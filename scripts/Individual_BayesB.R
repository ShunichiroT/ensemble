    ...
    for (p in 1:1){
      ...
      for (k in (cnt_sample-500+1):cnt_sample){
        ...
        for (i in 1:REPEAT){
          ...
          setwd(SAVE_PATH)
          write.csv(record, paste("BayesB_imputed_concat.csv",sep=''), row.names=FALSE)
          write.csv(result_prediction, paste("BayesB_imputed_concat_result.csv",sep=''), row.names=FALSE)
          write.csv(result_prediction_test, paste("BayesB_imputed_test_concat_result.csv",sep=''), row.names=FALSE)
          }
